...
:state_(state),hash(0),hashtable(0),ExpandableBlockStreamIteratorBase(barrier_number(2),serialized_section_number(1)),eftt_(0),memcpy_(0),memcat_(0){
//	sema_open_.set_value(1);
	initialize_expanded_status();
}

BlockStreamJoinIterator::BlockStreamJoinIterator()
:hash(0),hashtable(0),ExpandableBlockStreamIteratorBase(barrier_number(2),serialized_section_number(1)),eftt_(0),memcpy_(0),memcat_(0){
//	sema_open_.set_value(1);
	initialize_expanded_status();
}
...
thread_context* BlockStreamJoinIterator::createContext() {
	...
	jtc->l_block_for_asking_=BlockStreamBase::createBlock(state_.input_schema_left,state_.block_size_);
	jtc->l_block_stream_iterator_=jtc->l_block_for_asking_->createIterator();
